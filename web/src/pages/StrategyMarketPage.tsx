        ...
        <div className="w-full relative z-10">
          ...
          <div className="mb-12 border-b border-zinc-800 pb-8 relative">
            ...
            <div className="flex items-center gap-4 mb-4">
              <div className="bg-zinc-900 border border-zinc-700 p-3 rounded-none relative group overflow-hidden">
                <div className="absolute inset-0 bg-metroll-primary/20 opacity-0 group-hover:opacity-100 transition-opacity"></div>
                <Database className="w-8 h-8 text-metroll-primary relative z-10" />
              </div>
              <div>
                <h1 className="text-4xl font-bold tracking-tighter text-white uppercase glitch-text" data-text={t.title}>
                  {t.title}
                </h1>
                <p className="text-xs text-metroll-primary tracking-[0.3em] font-bold mt-1">
                // {t.subtitle}
                </p>
              </div>
            </div>
          ...
          <div className="flex flex-col md:flex-row gap-4 mb-8">
            ...
            <div className="relative flex-1 group">
              <div className="absolute -inset-0.5 bg-gradient-to-r from-metroll-primary/20 to-zinc-800/20 rounded opacity-0 group-hover:opacity-100 transition duration-500 blur"></div>
              <div className="relative bg-black flex items-center border border-zinc-800 group-hover:border-metroll-primary/50 transition-colors">
                <div className="pl-4 pr-3 text-zinc-500 group-hover:text-metroll-primary transition-colors">
                  <Terminal size={16} />
                </div>
                <input
                  type="text"
                  ...
                  className="w-full bg-transparent py-3 text-sm focus:outline-none placeholder-zinc-700 text-metroll-primary font-mono"
                />
                <div className="pr-4">
                  <div className="w-2 h-4 bg-metroll-primary animate-pulse"></div>
                </div>
              </div>
            </div>
                ...
                >
                  {selectedCategory === cat && (
                    <motion.div
                      ...
                      className="absolute inset-0 bg-metroll-primary"
          ...
          {isLoading && (
            <div className="flex flex-col items-center justify-center py-32 space-y-4">
              <div className="relative w-16 h-16">
                ...
                <div className="absolute inset-0 border-2 border-metroll-primary rounded-full border-t-transparent animate-spin"></div>
                <div className="absolute inset-0 flex items-center justify-center">
                  <Cpu size={24} className="text-metroll-primary/50" />
                </div>
              </div>
              <p className="text-metroll-primary text-xs tracking-widest animate-pulse">{t.loading}</p>
              <div className="flex gap-1">
                <div className="w-1 h-1 bg-metroll-primary rounded-full animate-bounce" style={{ animationDelay: '0s' }}></div>
                <div className="w-1 h-1 bg-metroll-primary rounded-full animate-bounce" style={{ animationDelay: '0.2s' }}></div>
                <div className="w-1 h-1 bg-metroll-primary rounded-full animate-bounce" style={{ animationDelay: '0.4s' }}></div>
              </div>
            </div>
          )}
                    ...
                    >
                      ...
                      <div className="p-6 relative">
                        ...
                        <h3 className={`text-lg font-bold mb-2 tracking-tight group-hover:${style.color} transition-colors uppercase truncate relative`}>
                          {strategy.name}
                          <span className="absolute -bottom-1 left-0 w-8 h-[2px] bg-zinc-800 group-hover:bg-metroll-primary transition-colors"></span>
                        </h3>
                        ...
                        <div>
                          {strategy.config_visible && strategy.config ? (
                            <button
                              ...
                              className="w-full py-2.5 text-[10px] font-bold font-mono uppercase tracking-widest border border-zinc-700 bg-black hover:bg-zinc-900 text-zinc-300 hover:text-metroll-primary hover:border-metroll-primary transition-all flex items-center justify-center gap-2 group/btn"
                            >
          ...
          {user && token && (
            ...
            >
              <div className="relative group cursor-pointer" onClick={() => window.location.href = '/strategy'}>
                <div className="absolute -inset-1 bg-gradient-to-r from-metroll-primary to-yellow-600 rounded blur opacity-25 group-hover:opacity-75 transition duration-1000 group-hover:duration-200"></div>
                <div className="relative px-8 py-4 bg-black border border-zinc-800 hover:border-metroll-primary/50 flex items-center gap-4 transition-all">
                  <Hexagon className="text-metroll-primary animate-spin-slow" size={24} />
                  <div className="text-left">
                    <div className="text-sm font-bold text-white uppercase tracking-wider group-hover:text-metroll-primary transition-colors">{t.shareYours}</div>
