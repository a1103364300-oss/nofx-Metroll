...
function MessageCard({ msg }: { msg: DebateMessage }) {
  ...
  return (
    <div
      className="p-3 rounded-lg hover:bg-bg-surface-base-lighter/60 transition-all border border-metroll-primary/20 backdrop-blur-sm bg-bg-surface-base-lighter/20"
    ...
    >
      ...
      >
        ...
        <span className="text-sm text-text-content-primary font-medium">{msg.ai_model_name}</span>
        <span className="text-xs text-text-content-tertiary">{p.nameEn}</span>
        ...
        <span className="text-xs text-metroll-primary font-medium">{msg.decision?.confidence || msg.confidence}%</span>
        {open ? <ChevronUp size={14} className="text-text-content-tertiary" /> : <ChevronDown size={14} className="text-text-content-tertiary" />}
      </div>
...
function VoteCard({ vote }: { vote: { ai_model_name: string; action: string; symbol?: string; confidence: number; leverage?: number; position_pct?: number; stop_loss_pct?: number; take_profit_pct?: number; reasoning: string } }) {
  ...
  return (
    <div className="bg-bg-surface-base-lighter/40 backdrop-blur-md rounded-xl p-4 border border-metroll-primary/20 hover:border-metroll-primary/50 transition-all shadow-lg hover:shadow-[0_0_20px_rgba(240,185,11,0.1)]">
      <div className="flex items-center justify-between mb-3">
        <div className="flex items-center gap-2">
          <AIAvatar name={vote.ai_model_name} size={28} />
          <div>
            <span className="text-text-content-primary font-semibold block">{vote.ai_model_name}</span>
            {vote.symbol && <span className="text-xs text-text-content-tertiary">{vote.symbol}</span>}
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-sm">
        <div className="flex justify-between"><span className="text-text-content-tertiary">Leverage</span><span className="text-text-content-primary font-semibold">{vote.leverage || '-'}x</span></div>
        <div className="flex justify-between"><span className="text-text-content-tertiary">Position</span><span className="text-text-content-primary font-semibold">{vote.position_pct ? `${(vote.position_pct * 100).toFixed(0)}%` : '-'}</span></div>
        <div className="flex justify-between"><span className="text-text-content-tertiary">SL</span><span className="text-red-400 font-semibold">{vote.stop_loss_pct ? `${(vote.stop_loss_pct * 100).toFixed(1)}%` : '-'}</span></div>
        <div className="flex justify-between"><span className="text-text-content-tertiary">TP</span><span className="text-green-400 font-semibold">{vote.take_profit_pct ? `${(vote.take_profit_pct * 100).toFixed(1)}%` : '-'}</span></div>
      </div>
      {vote.reasoning && (
        <p className="mt-3 text-xs text-text-content-tertiary leading-relaxed line-clamp-2 border-t border-metroll-primary/10 pt-2">{vote.reasoning}</p>
      )}
    </div>
  )
}
...
function CreateModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
      <div className="bg-bg-surface-base-lighter/90 backdrop-blur-xl rounded-xl w-full max-w-md p-6 border border-metroll-primary/30 shadow-2xl shadow-metroll-primary/10">
        <div className="flex justify-between items-center mb-4">
          <h3 className="text-lg font-bold text-text-content-primary">{t('createDebate', language)}</h3>
          <button onClick={onClose}><X size={20} className="text-text-content-tertiary" /></button>
        </div>

        <div className="space-y-3">
          <input
            value={name} onChange={e => setName(e.target.value)}
            placeholder={t('debateName', language)} className="w-full px-3 py-2 rounded-lg bg-bg-surface-base border border-metroll-primary/20 text-text-content-primary text-sm outline-none focus:border-metroll-primary"
          />

          {/* Strategy selector - moved up */}
          <select value={strategyId} onChange={e => setStrategyId(e.target.value)}
            className="w-full px-3 py-2 rounded-lg bg-bg-surface-base border border-metroll-primary/20 text-text-content-primary text-sm outline-none focus:border-metroll-primary">
          ...
          <div className="flex gap-2">
            ...
            {isStaticWithCoins ? (
              <select value={symbol} onChange={e => setSymbol(e.target.value)}
                className="flex-1 px-3 py-2 rounded-lg bg-bg-surface-base border border-metroll-primary/20 text-text-content-primary text-sm outline-none focus:border-metroll-primary">
                {staticCoins.map(coin => <option key={coin} value={coin}>{coin}</option>)}
              </select>
            ) : (
              <div className="flex-1 px-3 py-2 rounded-lg bg-bg-surface-base border border-metroll-primary/20 text-text-content-tertiary text-sm">
                {language === 'zh' ? '根据策略规则自动选择' : 'Auto-selected by strategy'}
              </div>
            )}
            <select value={maxRounds} onChange={e => setMaxRounds(+e.target.value)}
              className="px-3 py-2 rounded-lg bg-bg-surface-base border border-metroll-primary/20 text-text-content-primary text-sm outline-none focus:border-metroll-primary">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            {participants.map((p, i) => (
              <div key={i} className="flex items-center gap-1 px-2 py-1 rounded-lg text-xs"
                ...
                }} className="bg-transparent text-text-content-primary text-xs border-0 outline-none cursor-pointer">
                ...
                }} className="bg-transparent text-text-content-primary text-xs border-0 outline-none">
                  {aiModels.map(m => <option key={m.id} value={m.id}>{m.name}</option>)}
                </select>
                <button onClick={() => setParticipants(participants.filter((_, j) => j !== i))}
                  className="text-loss hover:text-red-300"><X size={12} /></button>
              </div>
            ))}
            <button onClick={addP} className="px-2 py-1 text-xs text-metroll-primary hover:bg-metroll-primary/10 rounded">
              + {t('addAI', language)}
            </button>
          </div>
        </div>
        ...
        <div className="flex gap-2 mt-4">
          <button onClick={onClose} className="flex-1 py-2 rounded-lg bg-bg-surface-base border border-metroll-primary/20 text-text-content-primary text-sm hover:bg-bg-surface-base-lighter transition-colors">{t('cancel', language)}</button>
          <button onClick={submit} disabled={creating}
            className="flex-1 py-2 rounded-lg bg-metroll-primary text-black font-semibold text-sm disabled:opacity-50 hover:bg-yellow-500 transition-colors">
...
export function DebateArenaPage() {
  ...
  return (
    <DeepVoidBackground className="h-full flex overflow-hidden relative" disableAnimation>
      ...
      <div className="w-56 flex-shrink-0 bg-bg-surface-base/80 backdrop-blur-md border-r border-metroll-primary/20 flex flex-col z-10">
        {/* New Debate Button */}
        <button onClick={() => setShowCreate(true)}
          className="m-2 py-2 rounded-lg bg-metroll-primary text-black font-semibold text-sm flex items-center justify-center gap-1 hover:bg-yellow-500 transition-colors">
          <Plus size={16} /> {t('newDebate', language)}
        </button>

        {/* Debate List */}
        <div className="px-2 py-1 text-xs text-text-content-tertiary font-semibold">{t('debateSessions', language)}</div>
        <div className="overflow-y-auto" style={{ maxHeight: '30%' }}>
          {debates?.map(d => (
            <div key={d.id} onClick={() => setSelectedId(d.id)}
              className={`p-2 cursor-pointer border-l-2 transition-all ${selectedId === d.id ? 'bg-metroll-primary/10 border-metroll-primary shadow-[inset_10px_0_20px_-10px_rgba(240,185,11,0.2)]' : 'border-transparent hover:bg-bg-surface-base-lighter/50'}`}>
              <div className="flex items-center gap-2">
                <span className={`w-2 h-2 rounded-full ${STATUS_COLOR[d.status]}`} />
                <span className="text-sm text-text-content-primary truncate flex-1">{d.name}</span>
              </div>
              <div className="text-xs text-text-content-tertiary mt-1">{d.symbol} · R{d.current_round}/{d.max_rounds}</div>
        ...
        <div className="flex-1 border-t border-metroll-primary/20 mt-2 overflow-hidden flex flex-col">
          <div className="px-2 py-2 text-xs text-text-content-tertiary font-semibold flex items-center gap-1">
          ...
          <div className="flex-1 overflow-y-auto px-2 space-y-2">
            {traders?.filter(tr => tr.is_running).map(tr => (
              <div key={tr.trader_id}
                ...
                className={`p-2 rounded-lg cursor-pointer transition-all ${traderId === tr.trader_id ? 'bg-profit/20 ring-1 ring-profit' : 'bg-bg-surface-base-lighter hover:bg-bg-surface-base-light'}`}>
                <div className="flex items-center gap-2">
                  <PunkAvatar seed={tr.trader_id} size={32} className="rounded-lg" />
                  <div className="flex-1 min-w-0">
                    <div className="text-sm text-text-content-primary font-medium truncate">{tr.trader_name}</div>
                    <div className="text-xs text-text-content-tertiary truncate">{tr.ai_model}</div>
                  </div>
            ...
            {traders?.filter(tr => !tr.is_running).slice(0, 3).map(tr => (
              <div key={tr.trader_id} className="p-2 rounded-lg bg-bg-surface-base-lighter opacity-50">
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-sm text-text-content-primary font-medium truncate">{tr.trader_name}</div>
                    <div className="text-xs text-text-content-tertiary">{t('offline', language)}</div>
                  </div>
                </div>
              </div>
            ))}
            {(!traders || traders.length === 0) && (
              <div className="text-xs text-text-content-tertiary text-center py-4">{t('noTraders', language)}</div>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        {detail ? (
          <>
            {/* Header Bar - Compact */}
            <div className="px-3 py-2 border-b border-metroll-primary/20 bg-bg-surface-base/60 backdrop-blur-md flex items-center gap-3 flex-shrink-0 shadow-sm">
              <span className={`w-2 h-2 rounded-full flex-shrink-0 ${STATUS_COLOR[detail.status]}`} />
              <span className="font-bold text-text-content-primary truncate">{detail.name}</span>
              <span className="text-metroll-primary font-semibold">{detail.symbol}</span>
              {strategyName && <span className="text-xs px-1.5 py-0.5 bg-purple-500/20 text-purple-400 rounded">{strategyName}</span>}
              <span className="text-xs text-text-content-tertiary">R{detail.current_round}/{detail.max_rounds}</span>
              ...
              <div className="flex gap-1 ml-2">
                {participants.map(p => {
                  ...
                  return (
                    <div key={p.id} className="flex items-center gap-1 px-1 py-0.5 rounded bg-bg-surface-base-lighter text-xs">
            ...
            <div className="flex-1 flex overflow-hidden">
              {Object.keys(rounds).length === 0 ? (
                ...
                <>
                  {/* Left - Rounds */}
                  <div className="flex-1 overflow-y-auto p-4 border-r border-metroll-primary/20">
                    <div className="text-sm text-text-content-tertiary font-semibold mb-3 flex items-center gap-2">
                  ...
                  {/* Right - Votes */}
                  {votes.length > 0 && (
                    <div className="w-[420px] flex-shrink-0 overflow-y-auto p-4 bg-bg-surface-base/30 backdrop-blur-sm">
                      <div className="text-sm text-text-content-tertiary font-semibold mb-3 flex items-center gap-2">
                        <Trophy size={16} className="text-metroll-primary" />
            ...
            {(decision || votes.length > 0) && (
              <div className="p-3 border-t border-metroll-primary/20 bg-gradient-to-r from-metroll-primary/10 via-bg-surface-base-lighter/50 to-orange-500/10 backdrop-blur-md flex items-center gap-4 flex-shrink-0">
                <div className="flex items-center gap-2">
                  <Trophy size={20} className="text-metroll-primary" />
                  <span className="text-sm text-text-content-tertiary">{t('consensus', language)}:</span>
                  {decision ? (
                    <>
                      {decision.symbol && <span className="text-metroll-primary font-bold mr-1">{decision.symbol}</span>}
                    ...
                    <span className="flex items-center gap-1 px-2 py-1 rounded font-bold bg-text-content-tertiary/20 text-text-content-tertiary">
                ...
                {decision && (
                  <div className="flex items-center gap-4 text-sm">
                    <span><span className="text-text-content-tertiary">{t('confidence', language)}</span> <span className="text-metroll-primary font-bold">{decision.confidence || 0}%</span></span>
                    {(decision.leverage ?? 0) > 0 && <span><span className="text-text-content-tertiary">{t('leverage', language)}</span> <span className="text-text-content-primary font-bold">{decision.leverage}x</span></span>}
                    {(decision.position_pct ?? 0) > 0 && <span><span className="text-text-content-tertiary">{t('position', language)}</span> <span className="text-text-content-primary font-bold">{((decision.position_pct ?? 0) * 100).toFixed(0)}%</span></span>}
                    {(decision.stop_loss ?? 0) > 0 && <span><span className="text-text-content-tertiary">SL</span> <span className="text-red-400 font-bold">{((decision.stop_loss ?? 0) * 100).toFixed(1)}%</span></span>}
                    {(decision.take_profit ?? 0) > 0 && <span><span className="text-text-content-tertiary">TP</span> <span className="text-green-400 font-bold">{((decision.take_profit ?? 0) * 100).toFixed(1)}%</span></span>}
                  </div>
                )}
                <div className="flex-1" />
                {decision && !decision.executed && (decision.action === 'open_long' || decision.action === 'open_short') && (
                  <button onClick={() => setExecId(detail.id)}
                    className="px-4 py-1.5 rounded-lg bg-metroll-primary text-black font-semibold text-sm flex items-center gap-1 hover:bg-yellow-500 transition-colors">
          ...
          <div className="flex-1 flex items-center justify-center text-text-content-tertiary">
      ...
      {execId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
          <div className="bg-bg-surface-base-lighter/90 backdrop-blur-xl rounded-xl w-full max-w-sm p-6 border border-metroll-primary/30 shadow-2xl shadow-metroll-primary/10">
            <h3 className="text-lg font-bold text-text-content-primary mb-4 flex items-center gap-2">
              <Zap className="text-metroll-primary" /> {t('executeTitle', language)}
            </h3>
            <select value={traderId} onChange={e => setTraderId(e.target.value)}
              className="w-full px-3 py-2 rounded-lg bg-bg-surface-base border border-metroll-primary/20 text-text-content-primary text-sm mb-3">
            ...
            <div className="text-xs text-yellow-300 bg-metroll-primary/10 p-2 rounded mb-3">
              ⚠️ {language === 'zh' ? '将使用账户余额执行真实交易' : 'Will execute real trade with account balance'}
            </div>
            <div className="flex gap-2">
              <button onClick={() => { setExecId(null); setTraderId('') }}
                className="flex-1 py-2 rounded-lg bg-bg-surface-base text-text-content-primary text-sm hover:bg-bg-surface-base-light transition-colors">{t('cancel', language)}</button>
