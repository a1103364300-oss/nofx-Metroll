...
const ACTION_CONFIG: Record<string, { color: string; bg: string; icon: string; label: string }> = {
  open_long: { color: 'var(--profit-green)', bg: 'rgba(14, 203, 129, 0.15)', icon: '📈', label: 'LONG' },
  open_short: { color: 'var(--loss-red)', bg: 'rgba(246, 70, 93, 0.15)', icon: '📉', label: 'SHORT' },
  close_long: { color: 'var(--metroll-warning)', bg: 'rgba(240, 185, 11, 0.15)', icon: '💰', label: 'CLOSE' },
  close_short: { color: 'var(--metroll-warning)', bg: 'rgba(240, 185, 11, 0.15)', icon: '💰', label: 'CLOSE' },
  hold: { color: 'var(--text-secondary)', bg: 'rgba(132, 142, 156, 0.15)', icon: '⏸️', label: 'HOLD' },
  wait: { color: 'var(--text-secondary)', bg: 'rgba(132, 142, 156, 0.15)', icon: '⏳', label: 'WAIT' },
}
...
function getConfidenceColor(confidence: number | undefined): string {
  ...
  if (confidence >= 80) return 'var(--profit-green)'
  if (confidence >= 60) return 'var(--metroll-warning)'
  return 'var(--loss-red)'
}
...
function ActionCard({ action, language, onSymbolClick }: { action: DecisionAction; language: Language; onSymbolClick?: (symbol: string) => void }) {
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <div
            ...
            style={{ background: action.success ? 'var(--profit-green)' : 'var(--loss-red)' }}
          />
        </div>
      </div>
      ...
      {isOpen && (
        <div className="grid grid-cols-4 gap-3 mt-3 pt-3" style={{ borderTop: '1px solid var(--border-default)' }}>
          {/* Entry Price */}
          ...
          {/* Stop Loss */}
          <div className="text-center">
            <div className="text-xs mb-1" style={{ color: 'var(--loss-red)' }}>
              {t('stopLoss', language)}
            </div>
            <div className="font-mono font-semibold" style={{ color: 'var(--loss-red)' }}>
          ...
          {/* Take Profit */}
          <div className="text-center">
            <div className="text-xs mb-1" style={{ color: 'var(--profit-green)' }}>
              {t('takeProfit', language)}
            </div>
            <div className="font-mono font-semibold" style={{ color: 'var(--profit-green)' }}>
          ...
          <div className="text-center">
            ...
            <div className="font-mono font-semibold" style={{ color: 'var(--metroll-warning)' }}>
              {action.leverage}x
            </div>
          </div>
        </div>
        ...
        <div className="mt-3 pt-3 flex items-center justify-between" style={{ borderTop: '1px solid var(--border-default)' }}>
          ...
          <div className="flex items-center gap-2">
            {(() => {
              ...
              const ratioColor = ratio >= 3 ? 'var(--profit-green)' : ratio >= 2 ? 'var(--metroll-warning)' : 'var(--loss-red)'
              return (
                <>
                  <div className="flex gap-1">
                    <span style={{ color: 'var(--loss-red)' }}>1</span>
                    <span style={{ color: 'var(--text-secondary)' }}>:</span>
                    <span style={{ color: 'var(--profit-green)' }}>{ratio.toFixed(1)}</span>
                  </div>
                  <div
    ...
    >
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div
          ...
          style={
            decision.success
              ? { background: 'rgba(14, 203, 129, 0.15)', color: 'var(--profit-green)', border: '1px solid rgba(14, 203, 129, 0.3)' }
              : { background: 'rgba(246, 70, 93, 0.15)', color: 'var(--loss-red)', border: '1px solid rgba(246, 70, 93, 0.3)' }
          }
        >
        ...
        {decision.cot_trace && (
          <div>
            ...
            >
              <div className="flex items-center gap-2">
                <span className="text-base">🧠</span>
                <span className="font-semibold" style={{ color: 'var(--metroll-warning)' }}>
                  {t('aiThinking', language)}
                </span>
              </div>
              <span
                className="text-xs px-2 py-0.5 rounded"
                style={{ background: 'rgba(240, 185, 11, 0.15)', color: 'var(--metroll-warning)' }}
              >
      ...
      {decision.execution_log && decision.execution_log.length > 0 && (
        <div
          ...
          style={{ background: '#0B0E11', border: '1px solid var(--border-default)' }}
        >
