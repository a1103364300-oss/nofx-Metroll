...
export function CoinSourceEditor({
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        <label className="block text-sm font-medium mb-3 text-text-content-primary">
          {t('sourceType')}
        </label>
          ...
          {sourceTypes.map(({ value, icon: Icon, color }) => (
            <button
              ...
              className={`p-4 rounded-lg border transition-all ${config.source_type === value
                ? 'ring-2 ring-metroll-primary bg-metroll-primary/10'
                : 'hover:bg-white/5 bg-bg-surface-base'
                } border-metroll-primary/20`}
            >
              <Icon className="w-6 h-6 mx-auto mb-2" style={{ color }} />
              <div className="text-sm font-medium text-text-content-primary">
                {t(value)}
              </div>
              <div className="text-xs mt-1 text-text-content-tertiary">
                {t(`${value}Desc`)}
              </div>
            </button>
          ))}
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-2 mb-3">
            {(config.static_coins || []).map((coin) => (
              <span
                key={coin}
                className="flex items-center gap-1 px-3 py-1.5 rounded-full text-sm bg-bg-surface-base-lighter text-text-content-primary"
              >
                {coin}
                {!disabled && (
                  <button
          ...
          {!disabled && (
            <div className="flex gap-2">
              <input
                ...
                className="flex-1 px-4 py-2 rounded-lg bg-bg-surface-base border border-metroll-primary/20 text-text-content-primary"
              />
              <button
                onClick={handleAddCoin}
                className="px-4 py-2 rounded-lg flex items-center gap-2 transition-colors bg-metroll-primary text-black hover:bg-yellow-500"
              >
      ...
      <div>
        <div className="flex items-center gap-2 mb-3">
          ...
          <label className="text-sm font-medium text-text-content-primary">
            {t('excludedCoins')}
          </label>
        </div>
        <p className="text-xs mb-3 text-text-content-tertiary">
          {t('excludedCoinsDesc')}
        </p>
        <div className="flex flex-wrap gap-2 mb-3">
          ...
          {(config.excluded_coins || []).length === 0 && (
            <span className="text-xs italic text-text-content-tertiary">
              {language === 'zh' ? '无' : 'None'}
            </span>
          )}
        </div>
        {!disabled && (
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 px-4 py-2 rounded-lg text-sm bg-bg-surface-base border border-metroll-primary/20 text-text-content-primary"
            />
            <button
              onClick={handleAddExcludedCoin}
              className="px-4 py-2 rounded-lg flex items-center gap-2 transition-colors text-sm bg-loss text-white hover:bg-red-600"
            >
      ...
      {config.source_type === 'ai500' && (
        <div
          className="p-4 rounded-lg bg-metroll-primary/5 border border-metroll-primary/20"
        >
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              <Zap className="w-4 h-4 text-metroll-primary" />
              <span className="text-sm font-medium text-text-content-primary">
          ...
          <div className="space-y-3">
            <label className="flex items-center gap-3 cursor-pointer">
              <input
                ...
                disabled={disabled}
                className="w-5 h-5 rounded accent-metroll-primary"
              />
              <span className="text-text-content-primary">{t('useAI500')}</span>
            </label>

            {config.use_ai500 && (
              <div className="flex items-center gap-3 pl-8">
                <span className="text-sm text-text-content-tertiary">
                  {t('ai500Limit')}:
                </span>
                <select
                  ...
                  disabled={disabled}
                  className="px-3 py-1.5 rounded bg-bg-surface-base border border-metroll-primary/20 text-text-content-primary"
                >
            ...
            <p className="text-xs pl-8 text-text-content-tertiary">
              {t('nofxosNote')}
            </p>
          </div>
        </div>
      ...
      {config.source_type === 'oi_top' && (
        <div
          className="p-4 rounded-lg bg-profit/5 border border-profit/20"
        >
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
          ...
          <div className="space-y-3">
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              />
              <span className="text-text-content-primary">{t('useOITop')}</span>
            </label>
      ...
      {config.source_type === 'oi_low' && (
        <div
          className="p-4 rounded-lg bg-loss/5 border border-loss/20"
        >
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
          ...
          <div className="space-y-3">
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              />
              <span className="text-text-content-primary">{t('useOILow')}</span>
            </label>
      ...
      {config.source_type === 'mixed' && (
        <div className="p-4 rounded-lg bg-blue-500/5 border border-blue-500/20">
          ...
          <div className="grid grid-cols-2 gap-3 mb-4">
            ...
            <div
              className={`p-3 rounded-lg border transition-all cursor-pointer ${
                config.use_ai500
                  ? 'bg-metroll-primary/10 border-metroll-primary/50'
                  : 'bg-bg-surface-base border-white/10 hover:border-metroll-primary/30'
              }`}
            ...
            >
              <div className="flex items-center gap-2 mb-2">
                <input
                  ...
                  disabled={disabled}
                  className="w-4 h-4 rounded accent-metroll-primary"
                  onClick={(e) => e.stopPropagation()}
                />
                <Database className="w-4 h-4 text-metroll-primary" />
                <span className="text-sm font-medium text-text-content-primary">AI500</span>
                <NofxOSBadge />
              </div>
              {config.use_ai500 && (
                <div className="flex items-center gap-2 mt-2 pl-6">
                  <span className="text-xs text-text-content-tertiary">Limit:</span>
                  <select
                    value={config.ai500_limit || 10}
                    onChange={(e) => {
                      e.stopPropagation()
                    ...
                    disabled={disabled}
                    className="px-2 py-1 rounded text-xs bg-bg-surface-base border border-metroll-primary/20 text-text-content-primary"
                    onClick={(e) => e.stopPropagation()}
                  >
                    {[5, 10, 15, 20, 30, 50].map(n => (
                      <option key={n} value={n}>{n}</option>
            ...
            <div
              className={`p-3 rounded-lg border transition-all cursor-pointer ${
                config.use_oi_top
                  ...
                  : 'bg-bg-surface-base border-white/10 hover:border-profit/30'
              }`}
            ...
            >
              ...
              <p className="text-xs text-text-content-tertiary pl-6 mb-1">
            ...
            <div
              className={`p-3 rounded-lg border transition-all cursor-pointer ${
                config.use_oi_low
                  ...
                  : 'bg-bg-surface-base border-white/10 hover:border-loss/30'
              }`}
            ...
            <div
              className={`p-3 rounded-lg border transition-all cursor-pointer ${
                (config.static_coins || []).length > 0
                  ...
                  : 'bg-bg-surface-base border-white/10 hover:border-gray-500/30'
              }`}
            >
              <div className="flex items-center gap-2 mb-2">
              ...
              <div className="flex flex-wrap gap-1 mt-2">
                {(config.static_coins || []).slice(0, 3).map((coin) => (
                  <span
                    key={coin}
                    className="flex items-center gap-1 px-2 py-0.5 rounded text-xs bg-bg-surface-base-lighter text-text-content-primary"
                  >
                    {coin}
                    {!disabled && (
                      <button
              ...
              {!disabled && (
                <div className="flex gap-1 mt-2">
                  <input
                    ...
                    onClick={(e) => e.stopPropagation()}
                    placeholder="BTC, ETH..."
                    className="flex-1 px-2 py-1 rounded text-xs bg-bg-surface-base border border-metroll-primary/20 text-text-content-primary"
                  />
                  <button
                    onClick={(e) => {
                      e.stopPropagation()
                    ...
                    className="px-2 py-1 rounded text-xs bg-metroll-primary text-black hover:bg-yellow-500"
                  >
          ...
          {(() => {
            ...
            return (
              <div className="p-2 rounded bg-bg-surface-base border border-white/10">
                <div className="flex items-center justify-between text-xs">
                  <span className="text-text-content-tertiary">{t('mixedSummary')}:</span>
                  <span className="text-text-content-primary font-medium">
                    {sources.join(' + ')}
                  </span>
                </div>
                <div className="text-xs text-text-content-tertiary mt-1">
